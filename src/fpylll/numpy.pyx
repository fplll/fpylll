...
def dump_mu(ndarray[double, ndim=2, mode="c"] mu not None, MatGSO m, int kappa, int block_size):
    u"""
     Dump a block of the GSO matrix μ into a a numpy array.

     :param mu: A numpy array of size (block_size*block_size) and type float64
...
def dump_r(ndarray[double, ndim=1, mode="c"] r not None, MatGSO m, int kappa, int block_size):
    u"""
     Dump a block of the GSO vector r into a a numpy array.

     :param mu: A numpy array of size (block_size) and type float64
     :param kappa: index of the beginning of the block
     :param block_size: size of the considered block
